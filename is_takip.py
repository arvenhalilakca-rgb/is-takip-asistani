...
st.markdown("""
    ...
    .chat-container {
        ...
        padding: 20px; border-radius: 15px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); min-height: 300px;
    }
    .message-bubble {
        background-color: #dcf8c6; padding: 10px 15px; border-radius: 8px;
        box-shadow: 0 1px 1px rgba(0,0,0,0.1); max-width: 80%; margin-bottom: 10px;
...
try:
    ...
    # Opsiyonel: Google Sheets (Sadece varsa bağlanır)
    try:
    ...
    try:
        with pdfplumber.open(pdf_file) as pdf:
            for page in pdf.pages:
                ...
                if not isim_match: isim_match = re.search(r"TİCARET UNVANI\s*[:\n]\s*(.*)", text)
                ...
                if not matrah_match: matrah_match = re.search(r"Matrah Toplamı.*?(\d{1,3}(?:\.\d{3})*,\d{2})", text, re.IGNORECASE)
        ...
        st.error(f"PDF Okuma Hatası: {e}"); return bos_df

    return pd.DataFrame(sonuclar) if sonuclar else bos_df
...
# --- 2. KDV ANALİZ ROBOTU ---
    ...
    if st.session_state['tasdik_data'] is None:
        st.warning("⚠️ Önce 'Veri Yükle' kısmından müşteri listesini yükleyin.")
        ...
        if df_res.empty:
            ...
            if not df_goster.empty:
                for i, row in df_goster.iterrows():
                    ...
                    with st.container():
                        c_detay, c_btn = st.columns([3, 1])
                        with c_detay:
                            st.markdown(f"<div class='risk-karti'><b>{musteri}</b><br>Kredi Kartı: {kk} | Beyan: {beyan}<br><b>FARK: {fark} TL</b></div>", unsafe_allow_html=True)
                        with c_btn:
                            tel = st.text_input(f"Personel Tel", key=f"t_{i}", placeholder="53X...")
...
# --- 3. PROFESYONEL MESAJ ---
    ...
    if st.session_state['tasdik_data'] is not None:
        ...
        with c_form:
            ...
            if tur == "Tek Müşteri":
                secilen = [st.selectbox("Müşteri", df_m["Ünvan / Ad Soyad"].tolist())]
            else:
                secilen = df_m["Ünvan / Ad Soyad"].tolist()
                st.warning(f"{len(secilen)} kişiye gidecek!")
            ...
            up_file = st.file_uploader("Dosya", type=["pdf","jpg","png","xlsx"]) if dosya_ekle else None

        with c_view:
            st.markdown("### Önizleme")
            orn_isim = secilen[0] if secilen else "İsim"
            final = icerik.replace("{isim}", str(orn_isim)).replace("{ay}", datetime.now().strftime("%B"))
            ...
            if st.button("🚀 GÖNDER", type="primary"):
                ...
                for i, m in enumerate(secilen):
                    ...
                    if not row.empty:
                        ...
                        kisi_msg = icerik.replace("{isim}", str(m)).replace("{ay}", datetime.now().strftime("%B"))
                        tels = numaralari_ayikla(tel)
                        for t in tels:
                            if up_file: 
                                up_file.seek(0)
                                whatsapp_dosya_gonder(t, up_file, up_file.name, kisi_msg)
                            else: whatsapp_text_gonder(t, kisi_msg)
                        if tels: basarili += 1
                    bar.progress((i+1)/len(secilen))
                st.success(f"{basarili} gönderim tamamlandı.")
    ...
    if st.session_state['tasdik_data'] is not None:
        ...
        if borclular.empty: st.success("Borçlu kalmadı!")
            ...
            for i, row in borclular.iterrows():
                ...
                c_inf, c_b = st.columns([3,1])
                with c_inf: st.markdown(f"<div class='kisi-karti'><b>{isim}</b><br>Borç: {tutar} TL</div>", unsafe_allow_html=True)
                with c_b:
                    if st.button("📲 Gönder", key=f"t_{i}"):
